...
export default function EditCampaignPage() {
  ...
  useEffect(() => {
    const loadCampaign = async () => {
      try {
        ...
        const campaign = await campaignsApi.get(projectId, campaignId);

        // Store service_type_id for later lookup
        if (campaign.service_type_id) {
          setCampaignServiceTypeId(campaign.service_type_id);
        }

        setFormData({
          target_type: campaign.target_type || 'customer',
          service_type_key: campaign.service_type_key || '',
          name: campaign.name || '',
          channel: campaign.channel || 'sms',
          message_template: campaign.message_template || '',
          email_subject: campaign.email_subject_template || campaign.email_subject || '',
          email_body: campaign.email_body_template || campaign.email_body || '',
          filter: campaign.segment_filter || campaign.filter || { logic: 'AND', conditions: [] },
          campaign_type: campaign.type || campaign.campaign_type || 'one_time',
          scheduled_at: formatDateForInput(campaign.scheduled_at),
          check_interval_minutes: campaign.check_interval_minutes || 1440,
          cooldown_days: campaign.cooldown_days || 30,
          run_start_hour: campaign.run_start_hour ?? 9,
          run_end_hour: campaign.run_end_hour ?? 18,
  ...
  // Update service_type_key when serviceTypes loads and we have a service_type_id
  useEffect(() => {
    if (campaignServiceTypeId && serviceTypes.length > 0 && !formData.service_type_key) {
      const matchingType = serviceTypes.find(st => st.id === campaignServiceTypeId);
      if (matchingType) {
        setFormData(prev => ({ ...prev, service_type_key: matchingType.key }));
      }
    }
  }, [campaignServiceTypeId, serviceTypes, formData.service_type_key]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await campaignsApi.update(projectId, campaignId, payload);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-indigo-50 dark:from-gray-950 dark:via-gray-900 dark:to-indigo-950 p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Update your campaign settings</p>
        </div>
            ...
            <div className="space-y-6">
              ...
              {formData.campaign_type === 'automated' && (
                <div className="space-y-4 p-4 rounded-2xl bg-purple-50 dark:bg-purple-900/10 border border-purple-200 dark:border-purple-800/30">
                  <div>
                    <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Check Interval</label>
                    <select
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Cooldown (days)</label>
                    <input
                      type="number"
                      min={1}
                      max={365}
                      ...
                      className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white focus:outline-none focus:border-indigo-500"
                    />
                    <p className="text-xs text-gray-500 mt-1">Prevent re-messaging the same target within this period</p>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Run Start Hour</label>
                      <select
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Run End Hour</label>
                      <select
                    ...
                    <div key={index} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setFormData({ ...formData, filter: { ...formData.filter, conditions: newConditions } });
                        }}
                      ...
                      <input
                        ...
                        placeholder="Value"
                        className="w-32 px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:border-indigo-500"
                      />
                      <button
                        onClick={() => {
                ...
                <button
                  onClick={() => {
                    setFormData({
                      ...
                      filter: {
                        ...formData.filter,
                        conditions: [...formData.filter.conditions, { field: '', operator: 'equals', value: '' }],
                      },
                    });
                  }}
                  className="w-full py-3 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-gray-500 hover:text-gray-700 hover:border-gray-400 transition-colors"
                >
                  + Add Condition
                </button>
              </div>
                  ...
                  <div className="p-4 rounded-2xl bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800/30 flex items-center gap-3">
                    ...
                    {previewLoading ? (
                      ...
                      <span className="text-purple-700 dark:text-purple-300 font-semibold">
                        {preview.count} {formData.target_type === 'customer' ? 'customers' : (selectedServiceType?.label?.en || formData.service_type_key)} match
                      </span>
                    ) : (
                        ...
                        {preview.sample.map((item) => (
                          <div key={item.id} className="px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            <div className="flex items-center justify-between">
                              <div>
                                <div className="font-medium text-gray-900 dark:text-white">{item.name || 'No name'}</div>
                              ...
                              {formData.target_type === 'service' && (
                                <div className="text-right">
                                  {item.expiry_at && <div className="text-sm text-gray-500">{item.expiry_at}</div>}
                                  {item.status && (
                  ...
                  {preview?.debug_sql && (
                    <div className="space-y-3">
                      <div className="p-3 rounded-lg bg-gray-900 overflow-x-auto">
                        ...
                        <pre className="text-xs text-green-400 font-mono whitespace-pre-wrap break-all">{preview.debug_sql}</pre>
                      </div>
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">Review your campaign settings before saving</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700">
                  ...
                  <div className="flex items-center gap-2">
                    {formData.channel === 'sms' && <Smartphone className="w-4 h-4 text-indigo-600" />}
                    {formData.channel === 'email' && <Mail className="w-4 h-4 text-emerald-600" />}
                    {formData.channel === 'both' && <Layers className="w-4 h-4 text-purple-600" />}
                ...
                <div className="p-4 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700">
                  ...
                  <div className="flex items-center gap-2">
                    {formData.campaign_type === 'one_time' ? <Zap className="w-4 h-4 text-indigo-600" /> : <RefreshCw className="w-4 h-4 text-purple-600" />}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                      {formData.target_type === 'customer' ? 'customers' : (selectedServiceType?.label?.en || formData.service_type_key)} will receive this campaign
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800/30 flex items-start gap-3">
                ...
                <p className="text-sm text-yellow-800 dark:text-yellow-300">
                  Changes will be saved. You can execute the campaign from the campaign detail page.
                </p>
              </div>
            </div>
          )}
